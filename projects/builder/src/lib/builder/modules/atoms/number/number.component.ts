...
export class NumberComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.field = this.cbpService.setDataEntryFieldName(this.field, DgTypes.NumericDataEntry);
    this.isEnabledUnit = this.field.requiredUnit ? true : false;
    this.subScription = this.controlService.styleModelobjValue.subscribe((res: any) => {
      if (res && res !== undefined && !this.controlService.isEmpty(res)) {
        ...
        this.styleObjectUnit = this.controlService.setStyles(res['levelNormal']);
        if(this.field.unitsRequired){
          this.styleObjectUnit['heigh'] ='auto !important';
        } 
  ...
  setUnits(event: any) {
    this.field.units = event.target.value;
  }
}
