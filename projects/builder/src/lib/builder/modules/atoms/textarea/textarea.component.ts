...
export class TextareaComponent implements OnInit {
  ...
  @ViewChild('viewTextarea', { static: false }) viewText: ElementRef | undefined;
  styleObject: any;
  ...
  constructor(public cbpService: CbpService, public auditService: AuditService,
    public controlService: ControlService, private cdref: ChangeDetectorRef,
    public hLService: HyperLinkService, public _buildUtil: BuilderUtil) { }

  ngOnInit() {
    // this.hLService.initEditorProps(this.field);
    this.field = this.cbpService.setDataEntryFieldName(this.field, DgTypes.TextAreaDataEntry);
  ...
  resizeTextArea(event: any) {
    this.field['height'] = event.height;
  }
  ngOnDestroy() {
