...
export class AngularResizableDirective implements OnInit, OnChanges, OnDestroy, AfterViewInit {
  ...
  private _origSize: Size = null as any;
  private _origPos: Position= null as any;

  private _defaultSize: Size = null as any;
  private _defaultPos: Position= null as any;


  /** Current Size and Position */
  private _currSize: Size = null as any;
  private _currPos: Position = null as any;

  /** Initial Size and Position */
  private _initSize: Size = null as any;
  private _initPos: Position = null as any;
  private  default_Resize_Val :any[] = [];

  /** Snap to gird */
  private _gridSize: IPosition = null as any;
   ...
   */
  private _helperBlock: HelperBlock = null as any;

  private draggingSub: Subscription = null as any;
   ...
   */
  @Input() rzContainment: string | HTMLElement = null as any;

  /**
   * Snaps the resizing element to a grid, every x and y pixels.
   * A number for both width and height or an array values like [ x, y ]
   */
  @Input() rzGrid: number | number[] = null as any;

  /** The minimum width the resizable should be allowed to resize to. */
  @Input() rzMinWidth: number = null as any;

  /** The minimum height the resizable should be allowed to resize to. */
  @Input() rzMinHeight: number = null as any;

  /** The maximum width the resizable should be allowed to resize to. */
  @Input() rzMaxWidth: number = null as any;

  /** The maximum height the resizable should be allowed to resize to. */
  @Input() rzMaxHeight: number = null as any;
  ...
  ngOnDestroy() {
    this.removeHandles();
    this._containment = null as any;
    this._helperBlock.dispose();
    this._helperBlock = null as any;
  }

  ngAfterViewInit() {
    const elm = this.el.nativeElement;
    this._initSize = Size.getCurrent(elm) as any;
    this._initPos = Position.getCurrent(elm) as any;
  ...
  public resetSize(size:Size) {
    this._currSize = Size.copy(size);
    this._currPos = Position.copy(this._initPos);
    this.doResetSize();
  }
  ...
  private updateContainment() {
    ...
    if (typeof this.rzContainment === 'string') {
      if (this.rzContainment === 'parent') {
        this._containment = this.el.nativeElement.parentElement as any;
      } else {
        this._containment = document.querySelector<HTMLElement>(this.rzContainment) as any;
      }
    } else {
  ...
  onMouseDown(event: MouseEvent | TouchEvent, handle: ResizeHandle) {
    ...
    if (!this._handleResizing) {
      this._origMousePos = Position.fromEvent(event) as any;
  ...
  private unsubscribeEvents() {
    ...
    this.draggingSub = null as any;
  }

  onMouseLeave() {
    if (this._handleResizing) {
      this.stopResize();
      this._origMousePos = null as any;
  ...
  onMouseMove(event: MouseEvent | TouchEvent) {
    if (this._handleResizing && this._resizable && this._origMousePos && this._origPos && this._origSize) {
      this.resizeTo(Position.fromEvent(event) as any) ;
      this.onResizing();
    }
  }
  ...
  private startResize(handle: ResizeHandle) {
    const elm = this.el.nativeElement;
    this._origSize = Size.getCurrent(elm) as any;
    this._defaultSize = Size.getCurrent(elm) as any;
    this._origPos = Position.getCurrent(elm) as any; // x: left, y: top
    this._defaultPos = Position.getCurrent(elm)as any;
  ...
  private stopResize() {
    ...
    this._handleResizing = null as any;
    this._direction = null as any;
    this._origSize = null as any;
    this._origPos = null as any;
    if (this._containment) {
      this.resetBounding();
    }
  }
  ...
  private doResize() {
    ...
    if (!this._direction || this._direction.n || this._direction.s || this._aspectRatio) {
      // if(this._defaultSize.height <this._currSize.height){
      //   this._currSize.height = this._defaultSize.height
      // }
      if(this.default_Resize_Val.length<1){
        this.default_Resize_Val.push(this._defaultSize);
        console.log("ggg",this.default_Resize_Val);
       }
      this.renderer.setStyle(container, 'height', this._currSize.height + 'px');
    }
    if (!this._direction || this._direction.w || this._direction.e || this._aspectRatio) {
      // if(this._defaultSize.width <this._currSize.width){
      //   this._currSize.width = this._defaultSize.width
      // }
  ...
  private doResetSize() {
    const container = this.el.nativeElement;
    if (!this._direction || this._direction.n || this._direction.s || this._aspectRatio) {
      console.log("laptop",this._defaultSize)
      if(this._defaultSize.height <this._currSize.height){
      //  this._currSize.height = this._defaultSize.height
      this._currSize.height=this.default_Resize_Val[0]?.height;
      }
      this.renderer.setStyle(container, 'height', this._currSize.height + 'px');
    }
    if (!this._direction || this._direction.w || this._direction.e || this._aspectRatio) {
      if(this._defaultSize.width <this._currSize.width){
     //   this._currSize.width = this._defaultSize.width
     this._currSize.width=this.default_Resize_Val[0]?.width;
      }
      this.renderer.setStyle(container, 'width', this._currSize.width + 'px');
    }
    this.renderer.setStyle(container, 'left', this._currPos.x + 'px');
