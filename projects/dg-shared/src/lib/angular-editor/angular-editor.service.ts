...
export class AngularEditorService {
  ...
  selectedText!: any;
  uploadUrl!: any;
  uploadWithCredentials!: any;
  fontLineHeight: any;
  subScriptOption: any = false;
  superScriptOption: any = false;
  constructor(
   ...
   */
  insertColor(color: string, where: string, update: string): void {
  ...
  setFontName(fontName: string) {
    // if(fontName === 'Normal'){ fontName = 'Poppins';}
  ...
  removeSelectedElements(tagNames: string) {
    ...
    this.getSelectedNodes().forEach((node: { nodeType: number; tagName: string; }) => {
      if (node.nodeType === 1 &&
